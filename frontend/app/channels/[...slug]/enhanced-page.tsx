...
const MessageItem = ({ 
  ...
  return (
    <div className={`group flex gap-4 px-4 py-0.5 hover:bg-[#2e3035]/60 relative mt-[1.0625rem] first:mt-2 ${menuOpen ? 'bg-[#2e3035]/60' : ''}`}>
      ...
      {/* Message Actions - Discord Style Floating Bar */}
      {canAct && (
      ...
      <div className="flex-1 overflow-auto bg-[#313338] scrollbar-thin scrollbar-thumb-[#1A1B1E] scrollbar-track-[#2B2D31] flex flex-col">
        {route.isMe ? (
          ...
          <div className="flex flex-col min-h-0 flex-1">
            {/* Messages Container - Pushes content to bottom if few messages */}
            <div className="flex-1 flex flex-col justify-end min-h-0">
               {/* Channel Welcome - Only show if we're at the top (which we are if few messages) 
                   In a real app with pagination, this would be conditional on "hasMoreBefore" being false
               */}
              ...
              {messagesLoading ? (
                <div className="flex items-center justify-center py-8">
                 ...
                 /* Already handled by Welcome message above */
                 null
              ) : (
